/**
 * Buy exactly 4,806.228545484410053444 TEST to return pool to 10,000 TEST
 */
...
async function main() {
  console.log("╔══════════════════════════════════════════════════════════════════╗");
  console.log("║   Buy Exact 4806.228545 TEST to Return Pool to 10,000 TEST      ║");
  console.log("╚══════════════════════════════════════════════════════════════════╝");
  console.log();
  ...
  // Calculate how much WETH needed to buy exactly 4806.228545484410053444 TEST
  // Using router's getAmountsIn function
  const testToBuy = hre.ethers.parseEther("4806.228545484410053444");

  console.log("🔄 Buying exactly 4806.228545484410053444 TEST with WETH...");
  console.log("   This will return pool to 10,000 TEST");
  console.log();

  // Use swapTokensForExactTokens to get exact output
  const deadline = Math.floor(Date.now() / 1000) + 300;

  // Deposit enough WETH for the swap (estimate high)
  await (await weth.deposit({ value: hre.ethers.parseEther("0.005") })).wait();
  await (await weth.approve(ROUTER, hre.ethers.parseEther("0.005"))).wait();

  const tx = await router.swapTokensForExactTokens(
    testToBuy,
    hre.ethers.parseEther("0.005"), // max WETH to spend
    [WETH, TEST_TOKEN],
    deployer.address,
    deadline
  );
  ...
  console.log();
  console.log("If pool is at 10,000 TEST, WETH should be > 0.01");
  console.log("The difference shows fees accumulated even when returning to start!");
  console.log();
