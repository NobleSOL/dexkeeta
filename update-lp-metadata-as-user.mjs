...
/**
 * Update LP STORAGE account metadata with shares (using user account)
 */
...
function getUserClient() {
  ...
  const client = KeetaNet.UserClient.fromNetwork(NETWORK, userAccount);
  return { client, account: userAccount };
}

// Main function
async function main() {
  console.log('🔄 Updating LP STORAGE Account Metadata (as User)\n');
  console.log('═══════════════════════════════════════════════════════════════\n');

  const { client, account } = getUserClient();

  console.log(`📡 Connected to Keeta ${NETWORK} network`);
  console.log(`👤 User account: ${account.publicKeyString.get()}\n`);
